...
```{r}
# Problem 1
files100=c(str_c('plot00',seq(1,9),'.png'),str_c('plot0',seq(10,99),'.png'),'plot100.png')
```
...
```{r}
length(str_subset(tolower(states),'i'))
```

2. How many states do not contain the letter `a`?
```{r}
length(states)-length(str_subset(tolower(states),'a'))
```
...
```{r}
str_subset(tolower(states),'^w.*[^aeiou]$')
```

6. Which states contain at least three `i`'s (e.g. Illinois)?
```{r}
at_least_three=subset(states,(str_count(tolower(states),'i')>=3))
at_least_three
```

7. Which states contain exactly five vowels (e.g. California)?
```{r}
five_vowel=subset(states,(str_count(tolower(states),'[aeiou]')==5))
five_vowel
```

8. Which states contain three consecutive vowels (e.g. Hawaii)?
```{r}
str_subset(tolower(states),'[aeiou]{3}')
```
...
# is_hex()
is_hex=function(x){
  result = str_detect(x,'^#[0-9a-fA-F]{6}') & nchar(x)==7
  return (result)
}
```
...
dna_to_aa=function(x){
  ...
  if (n%%3!=0){
    ...
    for (i in 1:(n/3)){
      term=str_sub(x,(3*(i-1)+1),(3*i))
      amino_acid=append(amino_acid,subset(dna$amino_acid,dna$dna_triplet==term))
    }
  }
  if (length(amino_acid)<(n/3)){
    print('Invalid DNA sequence')
  }else{
    paste(amino_acid,collapse = '')
  }
}
```
