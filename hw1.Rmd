...
```{r}
longest_dep_delay =max(na.exclude(flights$dep_delay))
longest_dep_delay
```

4. How long were the top 10 longest flights measured from departure to arrival? __longest_flights__
```{r}
flight_time =  flights$hour *60 + flights$minute
longest_flights = sort(flight_time,decreasing=TRUE)[1:10]
longest_flights
```

5. What is the average departure delay time for United Airlines (carrier UA)? __ua_delay__
```{r}
sub_set_= subset(flights, flights$carrier=='UA')
ua_delay = mean(na.exclude(sub_set_$dep_delay))
ua_delay
```

6. Order the months from highest average departure delay time to lowest.  __month_delay__
```{r}
delay_time = numeric(12)
for (i in 1:12){
month_=subset(flights,flights$month==i)
delay_time[i]= mean(na.exclude(month_$dep_delay))
}
order(delay_time,decreasing=TRUE)
```

7. How many unique planes does American Airlines (AA) have? __num_planes_aa__
```{r}
planes= table(flights$tailnum,flights$carrier)
num_planes_aa=0
for (i in 1:nrow(planes)){
  if (planes[i,2]==sum(planes[i,])){
    num_planes_aa=num_planes_aa+1
  }
}
num_planes_aa
```
...
```{r}
jfk_1000mi = nrow(subset(flights,flights$origin=='JFK'& flights$distance >1000))
jfk_1000mi
```

9. How many airlines fly from LGA to DFW? __num_lga_dfw__
```{r}
num_lga_dfw = nrow(subset(flights,flights$origin=='LGA' & flights$dest=='DFW'))
num_lga_dfw
```

10. Which airline had the most flights coming out of JFK? __most_flights_jfk__
```{r}
newset= subset(flights, flights$origin=='JFK')
table_=table(newset$carrier)
index <- which.max(table_)[[1]]
names(table_)[index]
```
